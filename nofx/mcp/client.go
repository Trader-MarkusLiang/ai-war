...
func (client *Client) parseMCPResponse(body []byte) (string, error) {
	...
	client.logger.Debugf("[MCP] Response preview: %s", preview)

	// Handle SSE streaming response (data: {...} format)
	// Check if response contains SSE data lines anywhere
	isSSE := strings.HasPrefix(bodyStr, "data:") ||
	...
	if isSSE {
		...
		if result := content.String(); result != "" {
			return result, nil
		}
		return "", fmt.Errorf("SSE response contained no content")
	}
