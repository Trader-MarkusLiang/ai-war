...
// globalDB holds the global database connection for package-level functions
var globalDB *sql.DB
...
type Store struct {
	...
	baselineStrategy *BaselineStrategyStore
...
func NewFromDB(db *sql.DB) *Store {
	globalDB = db // Set global DB for package-level functions
	return &Store{db: db}
}
...
func (s *Store) initTables() error {
	...
	if err := s.Evolution().initTables(); err != nil {
...
// BaselineStrategy gets baseline strategy storage
func (s *Store) BaselineStrategy() *BaselineStrategyStore {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.baselineStrategy == nil {
		s.baselineStrategy = NewBaselineStrategyStore(s.db)
	}
	return s.baselineStrategy
}
...
// Evolution gets evolution storage
func (s *Store) Evolution() *EvolutionStore {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.evolution == nil {
		s.evolution = &EvolutionStore{db: s.db}
	}
	return s.evolution
}
