...
func (s *AIModelStore) initTables() error {
	...
	s.db.Exec(`ALTER TABLE ai_models ADD COLUMN display_name TEXT DEFAULT ''`)
...
func (s *AIModelStore) List(userID string) ([]*AIModel, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&model.DisplayName, &createdAt, &updatedAt,
		)
		if err != nil {
			return nil, err
		}
...
func (s *AIModelStore) Update(userID, id string, enabled bool, apiKey, customAPIURL, customModelName, displayName string) error {
	...
	if err == nil {
		...
		if apiKey == "" {
			_, err = s.db.Exec(`
				UPDATE ai_models SET enabled = ?, custom_api_url = ?, custom_model_name = ?, display_name = ?, updated_at = datetime('now')
				WHERE id = ? AND user_id = ?
			`, enabled, customAPIURL, customModelName, displayName, existingID, userID)
		} else {
			encryptedAPIKey := s.encrypt(apiKey)
			_, err = s.db.Exec(`
				UPDATE ai_models SET enabled = ?, api_key = ?, custom_api_url = ?, custom_model_name = ?, display_name = ?, updated_at = datetime('now')
				WHERE id = ? AND user_id = ?
			`, enabled, encryptedAPIKey, customAPIURL, customModelName, displayName, existingID, userID)
		}
		return err
	}
	...
	logger.Infof("✓ Creating new AI model configuration: ID=%s, Provider=%s, Name=%s, DisplayName=%s", newModelID, provider, name, displayName)
	encryptedAPIKey := s.encrypt(apiKey)
	_, err = s.db.Exec(`
		INSERT INTO ai_models (id, user_id, name, provider, enabled, api_key, custom_api_url, custom_model_name, display_name, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
	`, newModelID, userID, name, provider, enabled, encryptedAPIKey, customAPIURL, customModelName, displayName)
	return err
}
