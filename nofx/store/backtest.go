...
func (s *BacktestStore) initTables() error {
	queries := []string{
		...
		// Baseline equity curve (traditional indicator strategy)
		`CREATE TABLE IF NOT EXISTS backtest_baseline_equity (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			run_id TEXT NOT NULL,
			ts INTEGER NOT NULL,
			equity REAL NOT NULL,
		...
		`CREATE INDEX IF NOT EXISTS idx_backtest_baseline_equity_run_ts ON backtest_baseline_equity(run_id, ts)`,
		`CREATE INDEX IF NOT EXISTS idx_backtest_baseline_trades_run_ts ON backtest_baseline_trades(run_id, ts)`,
	}
...
// UpdateRunState updates the state of a backtest run
func (s *BacktestStore) UpdateRunState(runID string, state string) error {
	_, err := s.db.Exec(`
		UPDATE backtest_runs
...
// UpdateBacktestRunState is a package-level function to update backtest run state
func UpdateBacktestRunState(runID string, state string) error {
	db := GetDB()
	if db == nil {
...
// SaveBaselineStrategyLink links a backtest run to a baseline strategy
func (s *BacktestStore) SaveBaselineStrategyLink(runID, baselineStrategyID string) error {
	_, err := s.db.Exec(`
		UPDATE backtest_runs
		SET baseline_strategy_id = ?
		WHERE run_id = ?
	`, baselineStrategyID, runID)
	return err
}

// GetBaselineStrategyID retrieves the baseline strategy ID for a run
func (s *BacktestStore) GetBaselineStrategyID(runID string) (string, error) {
	var id string
	err := s.db.QueryRow(`
		SELECT baseline_strategy_id
		FROM backtest_runs
		WHERE run_id = ?
	`, runID).Scan(&id)
