...
func calculateTimeframeSeries(klines []Kline, timeframe string, count int) *TimeframeSeriesData {
	...
	// Default EMA periods (20, 50) for live trading
	emaShort, emaLong := 20, 50
	...
	data := &TimeframeSeriesData{
		...
		EMAShortValues: make([]float64, 0, count),
		EMALongValues:  make([]float64, 0, count),
	...
	for i := start; i < len(klines); i++ {
		...
		// Calculate EMA short for each point
		if i >= emaShort-1 {
			ema := calculateEMA(klines[:i+1], emaShort)
			data.EMAShortValues = append(data.EMAShortValues, ema)
		}

		// Calculate EMA long for each point
		if i >= emaLong-1 {
			ema := calculateEMA(klines[:i+1], emaLong)
			data.EMALongValues = append(data.EMALongValues, ema)
		}
		...
		// Calculate Stoch RSI for each point (requires at least 14+14+3+3=34 bars)
		if i >= 33 {
			stochRSI := calculateStochRSI(klines[:i+1], 14, 14, 3, 3)
			data.StochRSI_K = append(data.StochRSI_K, stochRSI.K)
			data.StochRSI_D = append(data.StochRSI_D, stochRSI.D)
		}
	}
...
// StochRSIResult holds Stoch RSI calculation result
...
// smoothK: %K smoothing period (default 3)
// smoothD: %D smoothing period (default 3)
func calculateStochRSI(klines []Kline, lengthRSI, lengthStoch, smoothK, smoothD int) StochRSIResult {
	result := StochRSIResult{K: 50, D: 50}
	minLen := lengthRSI + lengthStoch + smoothK + smoothD
	if len(klines) < minLen {
		return result
	}

	// Step 1: Calculate RSI series
	rsiValues := make([]float64, 0, len(klines)-lengthRSI)
	for i := lengthRSI; i < len(klines); i++ {
		rsi := calculateRSI(klines[:i+1], lengthRSI)
		rsiValues = append(rsiValues, rsi)
	}

	if len(rsiValues) < lengthStoch+smoothK+smoothD {
		return result
	}

	// Step 2: Calculate Stochastic of RSI
	stochKRaw := make([]float64, 0, len(rsiValues)-lengthStoch+1)
	for i := lengthStoch - 1; i < len(rsiValues); i++ {
		window := rsiValues[i-lengthStoch+1 : i+1]
		...
		var stochK float64
		if maxRSI-minRSI > 0 {
			stochK = (rsiValues[i] - minRSI) / (maxRSI - minRSI) * 100
		} else {
			stochK = 50
		}
		stochKRaw = append(stochKRaw, stochK)
	}

	if len(stochKRaw) < smoothK+smoothD {
		return result
	}

	// Step 3: Smooth %K with SMA
	kSmoothed := make([]float64, 0, len(stochKRaw)-smoothK+1)
	for i := smoothK - 1; i < len(stochKRaw); i++ {
		sum := 0.0
		for j := 0; j < smoothK; j++ {
			sum += stochKRaw[i-j]
		}
		kSmoothed = append(kSmoothed, sum/float64(smoothK))
	}

	if len(kSmoothed) < smoothD {
		return result
	}

	// Step 4: Calculate %D as SMA of %K
	result.K = kSmoothed[len(kSmoothed)-1]
	sum := 0.0
	for i := len(kSmoothed) - smoothD; i < len(kSmoothed); i++ {
		sum += kSmoothed[i]
	}
	result.D = sum / float64(smoothD)
...
func formatTimeframeData(sb *strings.Builder, data *TimeframeSeriesData) {
	...
	if len(data.EMAShortValues) > 0 {
		sb.WriteString(fmt.Sprintf("EMA%d: %s\n", data.EMAShortPeriod, formatFloatSlice(data.EMAShortValues)))
	}

	if len(data.EMALongValues) > 0 {
		sb.WriteString(fmt.Sprintf("EMA%d: %s\n", data.EMALongPeriod, formatFloatSlice(data.EMALongValues)))
	}
...
// BuildTimeframeSeriesData constructs TimeframeSeriesData from klines (for backtesting multi-timeframe support)
// emaPeriods: EMA periods to use, e.g. [9, 26]. If nil or empty, defaults to [20, 50]
func BuildTimeframeSeriesData(tf string, klines []Kline, emaPeriods []int) *TimeframeSeriesData {
	if len(klines) == 0 {
		return nil
	}
	...
	emaShort, emaLong := 20, 50
	if len(emaPeriods) >= 2 {
		emaShort, emaLong = emaPeriods[0], emaPeriods[1]
	} else if len(emaPeriods) == 1 {
		emaShort = emaPeriods[0]
	}

	// Output limit for prompt (but use full data for calculation)
	maxOutputBars := 30

	result := &TimeframeSeriesData{
		Timeframe:      tf,
		EMAShortPeriod: emaShort,
		EMALongPeriod:  emaLong,
	}

	// Calculate indicators using full kline data
	allEMAShort := make([]float64, 0, len(klines))
	allEMALong := make([]float64, 0, len(klines))
	allMACD := make([]float64, 0, len(klines))
	allRSI7 := make([]float64, 0, len(klines))
	allRSI14 := make([]float64, 0, len(klines))
	allStochK := make([]float64, 0, len(klines))
	allStochD := make([]float64, 0, len(klines))

	for i := range klines {
		subset := klines[:i+1]
		allEMAShort = append(allEMAShort, calculateEMA(subset, emaShort))
		allEMALong = append(allEMALong, calculateEMA(subset, emaLong))
		allMACD = append(allMACD, calculateMACD(subset))
		allRSI7 = append(allRSI7, calculateRSI(subset, 7))
		allRSI14 = append(allRSI14, calculateRSI(subset, 14))

		stochRSI := calculateStochRSI(subset, 14, 14, 3, 3)
		allStochK = append(allStochK, stochRSI.K)
		allStochD = append(allStochD, stochRSI.D)
	}

	// Determine output range (last maxOutputBars)
	outputStart := 0
	outputLen := len(klines)
	if outputLen > maxOutputBars {
		outputStart = outputLen - maxOutputBars
		outputLen = maxOutputBars
	}

	// Fill output arrays with last N values
	result.Klines = make([]KlineBar, outputLen)
	result.MidPrices = make([]float64, outputLen)
	result.Volume = make([]float64, outputLen)

	for i := 0; i < outputLen; i++ {
		k := klines[outputStart+i]
		result.Klines[i] = KlineBar{
			Time:   k.CloseTime,
			Open:   k.Open,
			High:   k.High,
			Low:    k.Low,
		...
		result.MidPrices[i] = k.Close
		result.Volume[i] = k.Volume
	}

	// Copy last N indicator values
	result.EMAShortValues = allEMAShort[outputStart:]
	result.EMALongValues = allEMALong[outputStart:]
	result.MACDValues = allMACD[outputStart:]
	result.RSI7Values = allRSI7[outputStart:]
	result.RSI14Values = allRSI14[outputStart:]
	result.StochRSI_K = allStochK[outputStart:]
	result.StochRSI_D = allStochD[outputStart:]

	// Calculate ATR14 using full data
	result.ATR14 = calculateATR(klines, 14)
