...
func (s *Server) registerBacktestRoutes(router *gin.RouterGroup) {
	...
	router.POST("/force-resume", s.handleBacktestForceResume)
	...
	// Baseline (traditional indicator strategy) endpoints
	router.GET("/baseline/equity", s.handleBacktestBaselineEquity)
	router.GET("/baseline/trades", s.handleBacktestBaselineTrades)
	router.GET("/baseline/decisions", s.handleBacktestBaselineDecisions)
}
...
func (s *Server) handleBacktestStart(c *gin.Context) {
	...
	// Set strategy name as label if strategy was used
	if cfg.StrategyID != "" {
		if strategy, err := s.store.Strategy().Get(cfg.UserID, cfg.StrategyID); err == nil && strategy != nil {
			s.backtestManager.UpdateLabel(cfg.RunID, strategy.Name)
		}
	}
...
// handleBacktestForceResume 强制恢复回测，跳过状态检查
func (s *Server) handleBacktestForceResume(c *gin.Context) {
	if s.backtestManager == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "backtest manager unavailable"})
		return
	}
	...
	// 强制更新状态为running
	if err := s.backtestManager.ForceResume(req.RunID); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"message": "force resumed"})
		return
	}
	c.JSON(http.StatusOK, meta)
}
...
func (s *Server) handleBacktestRuns(c *gin.Context) {
	...
	// Batch load strategy IDs for all runs (single DB query)
	runStrategyIDs, _ := backtest.LoadStrategyIDsForRuns()

	// Collect unique strategy IDs that need name lookup
	strategyIDsToLookup := make(map[string]bool)
	for _, meta := range metas {
		if meta.Label == "" {
			if stratID, ok := runStrategyIDs[meta.RunID]; ok && stratID != "" {
				strategyIDsToLookup[stratID] = true
			}
		}
	}

	// Batch load strategy names (single DB query)
	strategyNameCache := make(map[string]string)
	if len(strategyIDsToLookup) > 0 {
		strategies, _ := s.store.Strategy().List(userID)
		for _, strat := range strategies {
			if strategyIDsToLookup[strat.ID] {
				strategyNameCache[strat.ID] = strat.Name
			}
		}
	}
	...
	for _, meta := range metas {
		...
		// Fill in label from strategy if not set (using cached data)
		if meta.Label == "" {
			if stratID, ok := runStrategyIDs[meta.RunID]; ok && stratID != "" {
				if name, ok := strategyNameCache[stratID]; ok {
					meta.Label = name
				}
			}
		}
...
// handleBacktestBaselineEquity returns baseline equity curve data
func (s *Server) handleBacktestBaselineEquity(c *gin.Context) {
	if s.backtestManager == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "backtest manager unavailable"})
		return
	}
	...
	points, err := backtest.LoadBaselineEquityPoints(runID)
	if err != nil {
...
// handleBacktestBaselineTrades returns baseline trade events
func (s *Server) handleBacktestBaselineTrades(c *gin.Context) {
	if s.backtestManager == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "backtest manager unavailable"})
		return
	}
	...
	events, err := backtest.LoadBaselineTradeEvents(runID)
	if err != nil {
		c.JSON(http.StatusOK, []backtest.TradeEvent{})
		return
	}
	c.JSON(http.StatusOK, events)
}

func (s *Server) handleBacktestBaselineDecisions(c *gin.Context) {
	if s.backtestManager == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "backtest manager unavailable"})
		return
	}
	...
	decisions, err := backtest.LoadBaselineDecisions(runID)
	if err != nil {
