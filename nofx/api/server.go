...
func NewServer(traderManager *manager.TraderManager, st *store.Store, cryptoService *crypto.CryptoService, backtestManager *backtest.Manager, port int) *Server {
	...
	// Set AI resolver for backtest resume
	if backtestManager != nil && st != nil {
...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		protected := api.Group("/", s.authMiddleware())
		{
			...
			// Baseline strategy management
			protected.GET("/baseline-strategies", s.handleListBaselineStrategies)
			protected.POST("/baseline-strategies", s.handleCreateBaselineStrategy)
			protected.GET("/baseline-strategies/rankings", s.handleGetBaselineRankings)
			protected.GET("/baseline-strategies/:id/performance", s.handleGetBaselinePerformance)
			protected.GET("/baseline-strategies/:id", s.handleGetBaselineStrategy)
			protected.PUT("/baseline-strategies/:id", s.handleUpdateBaselineStrategy)
			protected.DELETE("/baseline-strategies/:id", s.handleDeleteBaselineStrategy)
...
type SafeModelConfig struct {
	...
	DisplayName     string `json:"displayName"`     // User-defined display name (e.g., "Claude AI - 1")
}
...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	if exchangeCfg == nil {
		...
		case "weex":
			tempTrader = trader.NewWeexTrader(
				exchangeCfg.APIKey,
				exchangeCfg.SecretKey,
				exchangeCfg.Passphrase,
			)
...
func (s *Server) handleUpdateModelConfigs(c *gin.Context) {
	...
	for modelID, modelData := range req.Models {
		err := s.store.AIModel().Update(userID, modelID, modelData.Enabled, modelData.APIKey, modelData.CustomAPIURL, modelData.CustomModelName, modelData.DisplayName)
		if err != nil {
...
func (s *Server) handleGetSupportedExchanges(c *gin.Context) {
	...
	supportedExchanges := []SafeExchangeConfig{
		...
		{ExchangeType: "weex", Name: "WEEX", Type: "cex"},
