...
// LoadStrategyIDsForRuns returns a map of run_id -> strategy_id for all runs
func LoadStrategyIDsForRuns() (map[string]string, error) {
	if !usingDB() {
		return nil, nil
	}
	rows, err := persistenceDB.Query(`
		SELECT run_id, json_extract(config_json, '$.strategy_id')
		FROM backtest_runs
		WHERE config_json != '' AND config_json IS NOT NULL
	`)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&runID, &strategyID); err != nil {
			continue
		}
		if strategyID.Valid && strategyID.String != "" {
			result[runID] = strategyID.String
		}
	}
...
// ==================== Baseline DB Functions ====================

func appendBaselineEquityPointDB(runID string, point EquityPoint) error {
	_, err := persistenceDB.Exec(`
		INSERT INTO backtest_baseline_equity (run_id, ts, equity, available, pnl, pnl_pct, dd_pct, cycle)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, runID, point.Timestamp, point.Equity, point.Available, point.PnL, point.PnLPct, point.DrawdownPct, point.Cycle)
	return err
}

func appendBaselineTradeEventDB(runID string, event TradeEvent) error {
	_, err := persistenceDB.Exec(`
		INSERT INTO backtest_baseline_trades (run_id, ts, symbol, action, side, qty, price, fee, slippage, order_value, realized_pnl, leverage, cycle, position_after, liquidation, note)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, runID, event.Timestamp, event.Symbol, event.Action, event.Side, event.Quantity, event.Price, event.Fee, event.Slippage, event.OrderValue, event.RealizedPnL, event.Leverage, event.Cycle, event.PositionAfter, event.LiquidationFlag, event.Note)
	return err
}

func appendBaselineDecisionDB(runID string, cycle int, ts int64, equity, available float64, decisionJSON, reasoning string, signalCount int) error {
	_, err := persistenceDB.Exec(`
		INSERT OR REPLACE INTO backtest_baseline_decisions (run_id, cycle, ts, equity, available, decision_json, reasoning, signal_count)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, runID, cycle, ts, equity, available, decisionJSON, reasoning, signalCount)
	return err
}

func appendBaselineDecisionWithStatsDB(runID string, cycle int, ts int64, equity, available float64,
	decisionJSON, reasoning string, signalCount int, totalPnL, totalPnLPct, maxDrawdown float64,
	positionCount int, positionValue float64, positionsJSON string) error {
	_, err := persistenceDB.Exec(`
		INSERT OR REPLACE INTO backtest_baseline_decisions
		(run_id, cycle, ts, equity, available, decision_json, reasoning, signal_count,
		 total_pnl, total_pnl_pct, max_drawdown, position_count, position_value, positions_json)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, runID, cycle, ts, equity, available, decisionJSON, reasoning, signalCount,
		totalPnL, totalPnLPct, maxDrawdown, positionCount, positionValue, positionsJSON)
	return err
}

func loadBaselineEquityPointsDB(runID string) ([]EquityPoint, error) {
	rows, err := persistenceDB.Query(`
		SELECT ts, equity, available, pnl, pnl_pct, dd_pct, cycle
		FROM backtest_baseline_equity WHERE run_id = ? ORDER BY ts ASC
	`, runID)
	if err != nil {
		return nil, err
	}
...
func loadBaselineTradeEventsDB(runID string) ([]TradeEvent, error) {
	rows, err := persistenceDB.Query(`
		SELECT ts, symbol, action, side, qty, price, fee, slippage, order_value, realized_pnl, leverage, cycle, position_after, liquidation, note
		FROM backtest_baseline_trades WHERE run_id = ? ORDER BY ts ASC
	`, runID)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.Timestamp, &e.Symbol, &e.Action, &e.Side, &e.Quantity, &e.Price, &e.Fee, &e.Slippage, &e.OrderValue, &e.RealizedPnL, &e.Leverage, &e.Cycle, &e.PositionAfter, &e.LiquidationFlag, &note); err != nil {
			return nil, err
		}
		if note.Valid {
			e.Note = note.String
...
func loadBaselineDecisionsDB(runID string) ([]map[string]interface{}, error) {
	rows, err := persistenceDB.Query(`
		SELECT cycle, ts, equity, available, decision_json, reasoning, signal_count,
		       total_pnl, total_pnl_pct, max_drawdown, position_count, position_value, positions_json
		FROM backtest_baseline_decisions WHERE run_id = ? ORDER BY cycle ASC
	`, runID)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		var cycle, signalCount, positionCount int
		var ts int64
		var equity, available, totalPnL, totalPnLPct, maxDrawdown, positionValue float64
		var decisionJSON, reasoning, positionsJSON string

		if err := rows.Scan(&cycle, &ts, &equity, &available, &decisionJSON, &reasoning, &signalCount,
			&totalPnL, &totalPnLPct, &maxDrawdown, &positionCount, &positionValue, &positionsJSON); err != nil {
			return nil, err
		}
