...
// Baseline data paths
func baselineEquityLogPath(runID string) string {
	return filepath.Join(runDir(runID), "baseline_equity.jsonl")
}

func baselineTradesLogPath(runID string) string {
	return filepath.Join(runDir(runID), "baseline_trades.jsonl")
}

func baselineMetricsPath(runID string) string {
	return filepath.Join(runDir(runID), "baseline_metrics.json")
}
...
// appendBaselineEquityPoint appends an equity point to baseline data
func appendBaselineEquityPoint(runID string, point EquityPoint) error {
	if usingDB() {
		return appendBaselineEquityPointDB(runID, point)
	}
	return appendJSONLine(baselineEquityLogPath(runID), point)
}

// appendBaselineTradeEvent appends a trade event to baseline data
func appendBaselineTradeEvent(runID string, event TradeEvent) error {
	if usingDB() {
		return appendBaselineTradeEventDB(runID, event)
	}
	return appendJSONLine(baselineTradesLogPath(runID), event)
}

// appendBaselineDecision appends a baseline decision record
func appendBaselineDecision(runID string, cycle int, ts int64, equity, available float64, decisionJSON, reasoning string, signalCount int) error {
	if usingDB() {
		return appendBaselineDecisionDB(runID, cycle, ts, equity, available, decisionJSON, reasoning, signalCount)
	}
	return nil // File-based storage not implemented for decisions
}

// appendBaselineDecisionWithStats appends a baseline decision record with statistics
func appendBaselineDecisionWithStats(runID string, cycle int, ts int64, equity, available float64,
	...
	if usingDB() {
		return appendBaselineDecisionWithStatsDB(runID, cycle, ts, equity, available,
...
// LoadBaselineEquityPoints loads baseline equity points
func LoadBaselineEquityPoints(runID string) ([]EquityPoint, error) {
	if usingDB() {
		return loadBaselineEquityPointsDB(runID)
	}
	points, err := loadJSONLines[EquityPoint](baselineEquityLogPath(runID))
	if err != nil {
		return nil, err
	}
	sort.Slice(points, func(i, j int) bool {
...
// LoadBaselineTradeEvents loads baseline trade events
func LoadBaselineTradeEvents(runID string) ([]TradeEvent, error) {
	if usingDB() {
		return loadBaselineTradeEventsDB(runID)
	}
	return loadJSONLines[TradeEvent](baselineTradesLogPath(runID))
}

// LoadBaselineDecisions loads baseline decisions
func LoadBaselineDecisions(runID string) ([]map[string]interface{}, error) {
	if usingDB() {
		return loadBaselineDecisionsDB(runID)
	}
	return nil, nil // File-based storage not implemented
}

// DeleteRun deletes all data for a backtest run
func DeleteRun(runID string) error {
	if usingDB() {
		return deleteRunDB(runID)
	}
	// For file-based storage, remove the entire run directory
	dir := runDir(runID)
