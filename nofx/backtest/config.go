...
type BacktestConfig struct {
	...
	EnableBaseline       bool     `json:"enable_baseline"`        // Enable traditional indicator baseline for comparison
	BaselineStrategyID   string   `json:"baseline_strategy_id,omitempty"` // ID of baseline strategy to use
...
func (cfg *BacktestConfig) Validate() error {
	...
	// DecisionTimeframe must be one of the selected timeframes
	if cfg.DecisionTimeframe == "" {
		...
		// If DecisionTimeframe is not in user's selected timeframes, use the first selected timeframe
		found := false
		for _, tf := range cfg.Timeframes {
...
func (cfg *BacktestConfig) ToStrategyConfig() *store.StrategyConfig {
	...
	return &store.StrategyConfig{
		...
		Indicators: store.IndicatorConfig{
			...
			RSIPeriods:          []int{7, 14},
			ATRPeriods:          []int{14},
			StochRSILengthRSI:   14,
			StochRSILengthStoch: 14,
			StochRSISmoothK:     3,
			StochRSISmoothD:     3,
		},
