...
// ForceResume 强制恢复回测，先更新数据库状态为running，然后调用Resume
func (m *Manager) ForceResume(runID string) error {
	if runID == "" {
		return fmt.Errorf("run_id is required")
	}

	// 强制更新数据库状态为running
	if err := store.UpdateBacktestRunState(runID, "running"); err != nil {
		return fmt.Errorf("failed to update state: %w", err)
	}

	// 调用正常的Resume逻辑
	return m.Resume(runID)
}
...
func (m *Manager) Delete(runID string) error {
	...
	// Delete all data from database/filesystem
	if err := DeleteRun(runID); err != nil {
		return err
	}
	return nil
}
