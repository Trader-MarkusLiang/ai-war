...
type Runner struct {
	...
	// Baseline (traditional indicator strategy) for comparison
...
func NewRunner(cfg BacktestConfig, mcpClient mcp.AIClient) (*Runner, error) {
	...
	if cfg.EnableBaseline && strategyConfig != nil {
		r.baselineEnabled = true
		r.baselineAccount = NewBacktestAccount(cfg.InitialBalance, cfg.FeeBps, cfg.SlippageBps)
		r.baselineEngine = NewBaselineEngine(strategyConfig)
		r.baselineState = &BacktestState{
			Positions:      make(map[string]PositionSnapshot),
...
func (r *Runner) stepOnce() error {
	...
	// Execute Baseline strategy in parallel (if enabled)
	if r.baselineEnabled {
		r.executeBaselineStep(ts, marketData, priceMap, shouldDecide, callCount)
	}
...
func (r *Runner) handleCompletion() {
	...
	// Save baseline performance if baseline is enabled and strategy ID is provided
	if r.cfg.EnableBaseline && r.cfg.BaselineStrategyID != "" {
		// Link baseline strategy to this run
		db := store.GetDB()
		if db != nil {
			_, err := db.Exec(`
				UPDATE backtest_runs
				SET baseline_strategy_id = ?
				WHERE run_id = ?
			`, r.cfg.BaselineStrategyID, r.cfg.RunID)
			if err != nil {
				logger.Errorf("[Runner] Failed to save baseline strategy link: %v", err)
			}
		}

		// Save baseline performance metrics
		if err := r.saveBaselinePerformance(); err != nil {
			logger.Errorf("[Runner] Failed to save baseline performance: %v", err)
		}
	}
...
// ==================== Baseline Strategy Methods ====================

// executeBaselineStep executes the baseline (traditional indicator) strategy
func (r *Runner) executeBaselineStep(ts int64, marketData map[string]*market.Data, priceMap map[string]float64, shouldDecide bool, cycle int) {
	if !r.baselineEnabled || r.baselineEngine == nil || r.baselineAccount == nil {
		return
	}

	// Get baseline account state
	equity, _, _ := r.baselineAccount.TotalEquity(priceMap)
	available := r.baselineAccount.Cash()

	// Convert baseline positions to decision.PositionInfo format
	positions := r.convertBaselinePositions(priceMap)

	// 1. 首先检查挂单止损（每个bar都检查，基于OHLC数据）
	// 这模拟了真实交易所的止损单行为
	if len(positions) > 0 {
		stopDecisions := r.baselineEngine.CheckPendingStopLoss(marketData, positions)
		for _, dec := range stopDecisions {
			r.executeBaselineDecision(dec, priceMap, ts, cycle)
		}
		// 重新获取持仓（止损后可能已变化）
		positions = r.convertBaselinePositions(priceMap)
		equity, _, _ = r.baselineAccount.TotalEquity(priceMap)
	...
	// 2. 在决策点执行常规决策
	if shouldDecide {
		// Get deterministic decisions from baseline engine
		decisions := r.baselineEngine.MakeDecision(equity, available, marketData, positions)

		// Record baseline decision
		r.recordBaselineDecision(ts, cycle, equity, available, decisions, priceMap)
	...
	// Update baseline state and persist data
	r.updateBaselineState(ts, priceMap, cycle)
}

// convertBaselinePositions converts baseline account positions to decision.PositionInfo
func (r *Runner) convertBaselinePositions(priceMap map[string]float64) []decision.PositionInfo {
	positions := r.baselineAccount.Positions()
	result := make([]decision.PositionInfo, 0, len(positions))
	...
	for _, pos := range positions {
		...
		result = append(result, decision.PositionInfo{
			Symbol:           pos.Symbol,
			Side:             pos.Side,
			EntryPrice:       pos.EntryPrice,
...
// executeBaselineDecision executes a single baseline decision
func (r *Runner) executeBaselineDecision(dec decision.Decision, priceMap map[string]float64, ts int64, cycle int) {
	price := priceMap[dec.Symbol]
	if price <= 0 {
		return
	}
	...
	case "open_long":
		...
		if qty > 0 {
			_, _, _, err := r.baselineAccount.Open(dec.Symbol, "long", qty, dec.Leverage, price, ts)
			if err == nil {
				event = &TradeEvent{
					Timestamp:  ts,
					Symbol:     dec.Symbol,
					...
					Quantity:   qty,
					Price:      price,
					OrderValue: dec.PositionSizeUSD,
	...
	case "open_short":
		qty := dec.PositionSizeUSD / price
		if qty > 0 {
			_, _, _, err := r.baselineAccount.Open(dec.Symbol, "short", qty, dec.Leverage, price, ts)
			if err == nil {
				event = &TradeEvent{
					Timestamp:  ts,
					Symbol:     dec.Symbol,
	...
	case "close_long":
		...
		for _, pos := range r.baselineAccount.Positions() {
			if pos.Symbol == dec.Symbol && pos.Side == "long" {
				// Save quantity before closing (Close() will modify pos.Quantity)
				closedQty := pos.Quantity
				pnl, _, _, _ := r.baselineAccount.Close(dec.Symbol, "long", closedQty, price)
				event = &TradeEvent{
					Timestamp:   ts,
					Symbol:      dec.Symbol,
					Action:      dec.Action,
	...
	case "close_short":
		for _, pos := range r.baselineAccount.Positions() {
			if pos.Symbol == dec.Symbol && pos.Side == "short" {
				// Save quantity before closing (Close() will modify pos.Quantity)
				closedQty := pos.Quantity
				pnl, _, _, _ := r.baselineAccount.Close(dec.Symbol, "short", closedQty, price)
				event = &TradeEvent{
					Timestamp:   ts,
					Symbol:      dec.Symbol,
					Action:      dec.Action,
	...
	// Persist baseline trade event
	if event != nil {
		_ = appendBaselineTradeEvent(r.cfg.RunID, *event)
	}
}

// recordBaselineDecision records baseline decision for each cycle
func (r *Runner) recordBaselineDecision(ts int64, cycle int, equity, available float64, decisions []decision.Decision, priceMap map[string]float64) {
	...
	if len(decisions) > 0 {
		// Convert decisions to JSON
		if jsonBytes, err := json.Marshal(decisions); err == nil {
			decisionJSON = string(jsonBytes)
		}

		// Extract reasoning from first decision
		if decisions[0].Reasoning != "" {
			reasoning = decisions[0].Reasoning
		}

		signalCount = len(decisions)
	}

	// Calculate statistics
	initialBalance := r.baselineAccount.InitialBalance()
	totalPnL := equity - initialBalance
	totalPnLPct := (totalPnL / initialBalance) * 100

	// Get max drawdown from baseline state
	maxDrawdown := 0.0
	if r.baselineState != nil {
		maxDrawdown = r.baselineState.MaxDrawdownPct
	}
	...
	// Calculate total position value and build positions JSON
	positionsData := make([]map[string]interface{}, 0)
	for _, pos := range positions {
		...
		if currentPrice > 0 {
			...
			if pos.Margin > 0 {
				unrealizedPnLPct = (unrealizedPnL / pos.Margin) * 100
			}
		}
	...
	positionsJSON := "[]"
	if jsonBytes, err := json.Marshal(positionsData); err == nil {
		positionsJSON = string(jsonBytes)
	}

	// Persist baseline decision with statistics
	_ = appendBaselineDecisionWithStats(r.cfg.RunID, cycle, ts, equity, available,
		decisionJSON, reasoning, signalCount, totalPnL, totalPnLPct, maxDrawdown,
		positionCount, positionValue, positionsJSON)
}

// updateBaselineState updates baseline state and persists equity data
func (r *Runner) updateBaselineState(ts int64, priceMap map[string]float64, cycle int) {
	equity, _, _ := r.baselineAccount.TotalEquity(priceMap)
	available := r.baselineAccount.Cash()
	initialBalance := r.baselineAccount.InitialBalance()

	// Update max/min equity for drawdown calculation
	if equity > r.baselineState.MaxEquity {
		r.baselineState.MaxEquity = equity
	}
	if equity < r.baselineState.MinEquity {
		r.baselineState.MinEquity = equity
	}

	// Calculate drawdown
	drawdownPct := 0.0
	if r.baselineState.MaxEquity > 0 {
		drawdownPct = ((r.baselineState.MaxEquity - equity) / r.baselineState.MaxEquity) * 100
	}
	if drawdownPct > r.baselineState.MaxDrawdownPct {
		r.baselineState.MaxDrawdownPct = drawdownPct
	}
	...
	// Persist baseline equity point
	_ = appendBaselineEquityPoint(r.cfg.RunID, equityPoint)
}

// saveBaselinePerformance calculates and saves baseline performance metrics
func (r *Runner) saveBaselinePerformance() error {
	if r.baselineState == nil {
		return fmt.Errorf("baseline state is nil")
	}

	// Get baseline equity curve
	baselineEquity, err := LoadBaselineEquityPoints(r.cfg.RunID)
	if err != nil || len(baselineEquity) == 0 {
		return fmt.Errorf("failed to load baseline equity: %w", err)
	}

	// Get baseline trades
	baselineTrades, err := LoadBaselineTradeEvents(r.cfg.RunID)
	if err != nil {
		return fmt.Errorf("failed to load baseline trades: %w", err)
	}

	// Calculate metrics
	initialBalance := r.cfg.InitialBalance
	finalEquity := baselineEquity[len(baselineEquity)-1].Equity
	totalReturnPct := ((finalEquity - initialBalance) / initialBalance) * 100

	// Calculate win rate
	winRate := calculateWinRate(baselineTrades)

	// Calculate Sharpe ratio (simplified)
	sharpeRatio := calculateSharpeRatio(baselineEquity)

	// Create performance record
	perf := &store.BaselineStrategyPerformance{
		BaselineStrategyID: r.cfg.BaselineStrategyID,
		RunID:              r.cfg.RunID,
		Symbols:            r.cfg.Symbols,
		Timeframe:          r.cfg.DecisionTimeframe,
		StartTS:            r.cfg.StartTS,
		EndTS:              r.cfg.EndTS,
		InitialBalance:     initialBalance,
		FinalEquity:        finalEquity,
		TotalReturnPct:     totalReturnPct,
		MaxDrawdownPct:     r.baselineState.MaxDrawdownPct,
		SharpeRatio:        sharpeRatio,
		WinRate:            winRate,
		TotalTrades:        len(baselineTrades),
	}
	...
	return baselineStore.SavePerformance(perf)
}

// calculateWinRate calculates win rate from closing trades only
// Opening trades have RealizedPnL = 0 and should not be counted
func calculateWinRate(trades []TradeEvent) float64 {
	if len(trades) == 0 {
		return 0
	}
	...
	for _, trade := range trades {
		// Only count closing trades (close_long, close_short) and liquidations
		isClosing := strings.HasPrefix(trade.Action, "close") || trade.LiquidationFlag
		if !isClosing {
			continue
		}
		closingTrades++
	...
	return (float64(wins) / float64(closingTrades)) * 100
}

// calculateSharpeRatio calculates Sharpe ratio from equity curve
func calculateSharpeRatio(equity []EquityPoint) float64 {
	...
	for i := 1; i < len(equity); i++ {
		ret := (equity[i].Equity - equity[i-1].Equity) / equity[i-1].Equity
	...
	// Annualized Sharpe ratio (assuming 365 periods per year)
