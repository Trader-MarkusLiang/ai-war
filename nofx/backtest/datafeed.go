...
type DataFeed struct {
	...
	primaryTF     string
	longerTF      string
	emaPeriods    []int // EMA periods from strategy config
}

func NewDataFeed(cfg BacktestConfig) (*DataFeed, error) {
	// Get EMA periods from strategy config
	emaPeriods := []int{20, 50} // default
	if stratCfg := cfg.ToStrategyConfig(); stratCfg != nil {
		if len(stratCfg.Indicators.EMAPeriods) >= 2 {
			emaPeriods = stratCfg.Indicators.EMAPeriods
		}
	}
...
func (df *DataFeed) BuildMarketData(ts int64) (map[string]*market.Data, map[string]map[string]*market.Data, error) {
	...
	for _, symbol := range df.symbols {
		...
		tfSeriesData := make(map[string]*market.TimeframeSeriesData, len(df.timeframes))
		...
		for _, tf := range df.timeframes {
			...
			// Build TimeframeSeriesData for multi-timeframe support
			tfSeriesData[tf] = market.BuildTimeframeSeriesData(tf, series, df.emaPeriods)
		...
		// Attach all timeframe data to the primary result
		if primaryData, ok := result[symbol]; ok {
			primaryData.TimeframeData = tfSeriesData
		}
