...
export function AITradersPage({ onTraderSelect }: AITradersPageProps) {
  ...
  const handleSaveModelConfig = async (
    ...
    try {
      ...
      // 为新模型生成唯一ID（支持同一provider添加多个实例）
      let newModelId = modelId
      if (!existingModel) {
        // 新建模型时，生成唯一ID: provider_timestamp
        const timestamp = Date.now()
        newModelId = `${modelToUpdate.provider}_${timestamp}`
      }
      ...
      // 使用模型ID作为key（支持同一provider多个实例）
      const request = {
        models: Object.fromEntries(
          updatedModels.map((model) => [
            model.id, // 使用 id 而不是 provider，支持多实例
            {
              enabled: model.enabled,
              api_key: model.apiKey || '',
              custom_api_url: model.customApiUrl || '',
                  ...
                  <div className="flex items-center gap-2 md:gap-3">
                    ...
                    <div className="min-w-0">
                      <div
                        ...
                        style={{ color: '#EAECEF' }}
                      >
                        {model.displayName || getShortName(model.name)}
                      </div>
...
function ModelConfigModal({
  ...
  // 如果是编辑现有模型，初始化API Key、Base URL、Model Name和Display Name
  useEffect(() => {
    if (editingModelId && selectedModel) {
      setApiKey(selectedModel.apiKey || '')
      setBaseUrl(selectedModel.customApiUrl || '')
      setModelName(selectedModel.customModelName || '')
      setDisplayName(selectedModel.displayName || '')
    }
                ...
                >
                  ...
                  {availableModels.map((model) => (
                    <option key={model.id} value={model.id}>
                      {model.displayName || getShortName(model.name)} ({model.provider})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <>
                <div>
                  <label
                    ...
                    style={{ color: '#EAECEF' }}
                  >
                    {t('customDisplayName', language)}
                  </label>
                  <input
                    type="text"
                    value={displayName}
                    onChange={(e) => setDisplayName(e.target.value)}
                    placeholder={t('customDisplayNamePlaceholder', language)}
                    className="w-full px-3 py-2 rounded"
                    style={{
                      background: '#0B0E11',
                      border: '1px solid #2B3139',
                  ...
                  <div className="text-xs mt-1" style={{ color: '#848E9C' }}>
                    {t('customDisplayNameHint', language)}
                  </div>
                </div>
